...
public class FormEditorEJB extends AbstractEJB {
	...
	@Inject
	private ListMaritacaListDAO listMaritacaListDAO;
	@Inject
	private UserDAO userDAO;
	@Inject
	private FormDAO formDAO;
	@Inject
	private FormEditorDAO formEditorDAO;
	...
	public void saveNewForm(FormDTO formDTO) {
		...
		form.setUser(user);
		String[] formIdAsString = formDTO.getXml().split("<(/)*formid>");
		formIdAsString[1] = TimeUUIDUtils.getUniqueTimeUUIDinMillis()
				.toString();
		form.setKey(formIdAsString[1]);
		StringBuilder xmlAlterado = new StringBuilder();
		xmlAlterado.append(formIdAsString[0]).append("<formid>");
		xmlAlterado.append(formIdAsString[1]).append("</formid>");
		xmlAlterado.append(formIdAsString[2]);
		form.setXml(xmlAlterado.toString());
		form.setKey(formIdAsString[1]);

		formIdAsString = xmlAlterado.toString().split("<(/)*userid>");
		formIdAsString[1] = formDTO.getUser().toString();
		xmlAlterado = new StringBuilder();
		xmlAlterado.append(formIdAsString[0]).append("<userid>");
		xmlAlterado.append(formIdAsString[1]).append("</userid>");
		xmlAlterado.append(formIdAsString[2]);
		form.setXml(xmlAlterado.toString());
	 ...
	 * @return FormDTO
	 */
	public FormDTO getFormDTOByUserDTOAndFormDTO(FormDTO formDTO,
			UserDTO userDTO) {
		...
		if (permission != null && permission.getRead()) {
			...
			throw new AuthorizationDenied(Document.FORM, form.getKey(),
	...
	public void updateForm(FormDTO formDTO, UserDTO userDTO) {
		// TODO: verifyEntity(formDTO.getUserKey());
		// verifyNullProperties(formDTO); and keyUser
		Form originalForm = (Form) verifyReturnNullValuesInDB(formDTO);
		User user = (User) verifyReturnNullValuesInDB(userDTO);

		Permission permission = super.getPermission(originalForm,
				userDTO.getKey(), Document.FORM);
		if (permission != null && permission.getUpdate()) {
			...
			throw new AuthorizationDenied(Document.FORM, originalForm.getKey(),
					user.getKey(), Operation.UPDATE);
		}
	}
	...
	private <T> Object verifyReturnNullValuesInDB(T obj) {
		if (obj instanceof FormDTO) {
			...
			if (form == null) {
				throw new IllegalArgumentException("Form " + formDTO.getKey()
	 ...
	 * @return
	 */
	public Boolean updateFormFromPolicyEditor(FormDTO formDTO, UserDTO userDTO,
			List<MaritacaListDTO> usedItems) {
		// Form originalForm = (Form) verifyReturnNullValuesInDB(formDTO);//
		Form originalForm = formDAO.getFormByKey(formDTO.getKey(), false);
		User user = (User) verifyReturnNullValuesInDB(userDTO);
		...
		if (permission != null && permission.getUpdate()) {
			...
			if (originalForm.isShared()) {
				formEditorDAO.createOrUpdateFormAccessible(
						originalForm,
						user,
						checkDeletedLists(originalLists,
								originalForm.getLists()));
			}
			// Build the app mobile
	...
	private List<UUID> checkDeletedLists(List<UUID> currentLists,
			List<UUID> newLists) {
	...
	public List<MaritacaListDTO> getOwnerOfMaritacaListByPrefix(String prefix) {
		...
		Collection<MaritacaList> lists = listMaritacaListDAO
				.maritacaListsStartingWith(prefix);
		for (MaritacaList list : lists) {
			listsDto.add(UtilsBusiness.reflectClasses(list,
					MaritacaListDTO.class));
		}
		return listsDto;
	}

	public MaritacaListDTO searchMaritacaListByName(String selectedItem) {
		MaritacaList list = formEditorDAO
				.searchMaritacaListByName(selectedItem);
		if (list != null) {
	...
	public List<MaritacaListDTO> populateFormSharedList(FormDTO formDTO) {
		...
		if (form.getLists() != null) {
			for (UUID id : form.getLists()) {
				MaritacaList mList = listMaritacaListDAO
						.getMaritacaListById(id);
				if (mList != null) {
					lstItems.add(UtilsBusiness.reflectClasses(mList,
							MaritacaListDTO.class));
				}
			}
		}
	...
	public FormDTO getFormDTOById(UUID uuid, UserDTO userDTO) {
		...
		if (form != null) {
			Permission permission = super.getPermission(form, form.getUser()
					.getKey(), Document.FORM);
			if (permission != null && permission.getRead()) {
				formDTO = new FormDTO();
				formDTO.setKey(form.getKey());
				formDTO.setTitle(form.getTitle());
